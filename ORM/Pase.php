...
class Pase{
    ...
    public function BuscarXCantJuego($cant){
        $base = new BaseDatos();
        $arreglo=array();

        $sql = "SELECT * FROM pase WHERE cantidadjuegos=".$cant;
        $resp= false;
        if($base->Iniciar()){
            if($base->Ejecutar($sql)){
                while($row=$base->Registro()){
                    ...
                    $objPase->cargar($row['idpase'],$unVisitante,$row['fechaemision'],$row['cantidadjuegos'],$row['conaptitud'],$unParque);

                    /* $this->setIDPase($$row['idpase']);
                    $this->setVisitante($unVisitante);
                    $this->setFechaEmision($row['fechaemision']);
                    $this->setCantidadJuegos($row['cantidadjuegos']);
                    $this->setConAptitud($row['conaptitud']); */
    ...
    public function BuscarXFecha($fecha){
        ...
        $sql = "SELECT * FROM pase WHERE fechaemision='".$fecha."'";
        $resp= false;
        if($base->Iniciar()){
            if($base->Ejecutar($sql)){
                while($row=$base->Registro()){
    ...
    public function BuscarXCondicion($condicion){
        $base = new BaseDatos();
        $arreglo=array();

        $sql = "SELECT * FROM pase WHERE ".$condicion;
        $resp= false;
        if($base->Iniciar()){
            if($base->Ejecutar($sql)){
                while($row=$base->Registro()){
